...
if 'use_synthetic_data_only' not in st.session_state:
    st.session_state.use_synthetic_data_only = False

# VADER-based Real News Sentiment Analysis Functions

def get_real_news_for_ticker(ticker: str, days: int = 7):
    """Fetch real news for a ticker using multiple sources with improved rate limiting"""
    
    # Check if we should use synthetic data only (to avoid API limits)
    if st.session_state.use_synthetic_data_only:
        st.info(f"ℹ️ Using synthetic news data for {ticker} to avoid API limits")
        return generate_enhanced_synthetic_news(ticker, days)
    ...
    if cache_key in st.session_state.news_cache:
        ...
        # Increased cache time to 4 hours to reduce API calls
    ...
    try:
        ...
        st.warning(f"API rate limit reached for {ticker}. Switching to synthetic data mode.")
        # Automatically enable synthetic-only mode when hitting rate limits
        st.session_state.use_synthetic_data_only = True
        news_data = generate_enhanced_synthetic_news(ticker, days)
...
def fetch_yahoo_finance_news(ticker: str):
    """Fetch news from Yahoo Finance with better error handling and rate limiting"""
    try:
        import yfinance as yf
        
        # Add random delay to prevent hammering the API
        time.sleep(random.uniform(0.1, 0.5))
        
        stock = yf.Ticker(ticker)
        
        # Use a try-catch for the specific news call
        try:
            news = stock.news
        except Exception as e:
            # If news fails due to rate limit, return None to fallback to synthetic
            if "rate" in str(e).lower() or "limit" in str(e).lower() or "blocked" in str(e).lower():
                st.warning(f"Yahoo Finance rate limit reached for {ticker} news. Using synthetic data.")
                return None
            raise e
        
        news_data = []
        for article in news[:15]:  # Reduced from 20 to 15 to be gentler on API
        ...
        if any(keyword in error_msg for keyword in ['rate', 'limit', 'blocked', 'exceeded', 'quota']):
            st.warning(f"Yahoo Finance API limit reached. Using synthetic data for {ticker}.")
        return None
...
def main():
    ...
    # Show rate limit information if in synthetic mode
    if st.session_state.use_synthetic_data_only:
        st.warning("""
        ⚠️ **Synthetic Data Mode Active** - The app is currently using synthetic data to avoid Yahoo Finance API rate limits.
        
        💡 **To restore real-time data:**
        1. Wait 10-15 minutes for rate limits to reset
        2. Click "🔄 Reset API Mode" in the sidebar
        3. Clear caches if needed
        """)
    
    # Add helpful information about API limits
    with st.expander("ℹ️ Having Issues with API Limits? Click here for help"):
        st.markdown("""
        **🚨 If you're seeing "fair-use limit" errors:**
        
        **Immediate Solutions:**
        1. ✅ **Enable Synthetic Mode** - Use the checkbox in the sidebar to avoid all API calls
        2. ✅ **Wait 10-15 minutes** - Yahoo Finance rate limits usually reset quickly
        3. ✅ **Clear all caches** - Use the "Clear All Caches" button in the sidebar
        
        **Why this happens:**
        - Yahoo Finance has free usage limits to prevent abuse
        - Multiple rapid requests can trigger temporary blocks
        - The app now has improved caching and rate limiting to prevent this
        
        **What's been improved:**
        - ⏰ **Longer caching**: Analysis cached for 2 hours instead of 1
        - 🛡️ **Rate limiting**: Automatic delays between API calls
        - 🤖 **Synthetic fallback**: High-quality synthetic data when APIs are unavailable
        - 📊 **Better error handling**: Clear messages when limits are hit
        """)
    ...
    with st.sidebar:
        ...
        **🛡️ VADER + Bias Reduction:**
        ...
        # Toggle for synthetic data mode
        use_synthetic = st.checkbox(
            "🤖 Use Synthetic Data Only (Avoid API Calls)", 
            value=st.session_state.use_synthetic_data_only,
            help="Enable this to avoid hitting Yahoo Finance rate limits"
        )
        st.session_state.use_synthetic_data_only = use_synthetic
        
        if use_synthetic:
            st.info("ℹ️ Synthetic data mode enabled - no API calls will be made")
        else:
            st.info("🌐 Real-time API mode enabled - using Yahoo Finance data")
        ...
        with col2:
            if st.button("🗑️ Clear News Cache"):
                st.session_state.news_cache = {}
        ...
        if st.button("🔄 Reset API Mode"):
            st.session_state.use_synthetic_data_only = False
            st.success("API mode reset - will try real data again!")
        ...
        if st.session_state.news_cache:
            st.write(f"📰 **News cache:** {len(st.session_state.news_cache)} entries")
        else:
            st.write("📰 **News cache:** Empty")
